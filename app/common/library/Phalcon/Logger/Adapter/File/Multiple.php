...
class Multiple extends FileLogger implements AdapterInterface
{
    ...
    /**
     * Set directory path for saving the log files.
     *
     * @param  string $path    Directory path for saving the log files.
     * @throws \Phalcon\Logger\Exception
     * @return this
    ...
    {
        if(sizeof($options)>0 && sizeof($this->options)>0){
     ...
     */
    protected function rotateFile($filepath)
    {
        clearstatcache(true, $filepath);

        if (!file_exists($filepath) ||
            filesize($filepath) < (int) $this->options['size']
        ) {
            return null;
        }

        $rotate = (int) $this->options['rotate'];
        if ($rotate === 0) {
